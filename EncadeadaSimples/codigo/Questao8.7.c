...
void separarPorGenero(ListaDeCompras *primeiroItem)
{
    ...
    while (atual != NULL)
    {
        ...
        {
            if (generos == NULL)
            { // se a lista ainda não existe, cria ela
    ...
    for (int i = 0; i < index; i++)
    {
        ...
        while (atual != NULL)
        {
            ...
            {
                if (isFirst)
                { // se for o primeiro item daquele genero, cria a lista
                    separadas[i] = criarItem(atual->produto);
                    isFirst = false;
                }
                else
                { // se não for o primeiro item, apenas insere na lista
...
int main()
{
    ...
    inserir(primeiroItem, prodNovo);
    primeiroItem = retirar(primeiroItem, 6);
